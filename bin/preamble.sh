...
# Date prefix for new flat spec files: yy-mm-dd
_TODAY=$(date +%y-%m-%d)

# Detect flat file convention: specs/yy-mm-dd-name.md (no branch subdir)
...
if [ -d "$_BRANCH_SPEC_DIR" ]; then
  ...
  # Flat dated layout: find the latest spec file (exclude companion files)
  _SPEC_STYLE="flat"
  _LATEST_SPEC=$(ls "$_REPO_ROOT/specs"/[0-9]*.md 2>/dev/null | grep -v '\-plan\|\-tasks\|\-research\|\-checklist' | tail -1)
